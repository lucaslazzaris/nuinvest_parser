...
class NoteParser:
  ...
  def get_note_info(self):
    ...
    for page in range(0, pages_number):
      ...
      raw_stocks, date, cblc_taxes, b3_taxes, doc_no = self.parser.get_data(text_content)
      broker = self.parser.broker

      stocks = [self.parser.sanitize_stock(stock) for stock in raw_stocks]
      stocks_with_taxes = self.parser.create_stock_with_taxes(stocks, cblc_taxes, b3_taxes)
